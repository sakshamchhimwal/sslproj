...
/**
    ...
    */
    ...
    strip_code = StripCode(code);
    tab_array = TabCount(code);
    if_dict = checkIfInCode(strip_code, tab_array);
    for_list = checkForInCode(strip_code, tab_array);
    else_array = checkElseInCode(strip_code, tab_array);
    fun_dict = checkDefInCode(strip_code, tab_array);
    while_array = checkWhileInCode(strip_code, tab_array);
    func_name_array = get_function_name(strip_code, def_dict);

function keyPressed(e) {
    if (e["key"] === "Enter") {
        code = code.split("\n").join("\\join");
        document.cookie = "xcode = " + code + ";SameSite=None; Secure";
        console.log(document.cookie);
    }
}
</script>
